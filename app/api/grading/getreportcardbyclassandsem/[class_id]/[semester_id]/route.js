  ...
  try {
    ...
    const query = `
      WITH class_subjects AS (
        ...
        WHERE sg.class_id = $1 AND sg.semester_id = $2 AND sg.status = 'active'
      ),
      student_averages AS (
        SELECT 
          sg.student_id,
      ...
      student_subject_grades AS (
        ...
        LEFT JOIN student_grades sg ON s.student_id = sg.student_id AND cs.subject_id = sg.subject_id
          AND sg.class_id = $1 AND sg.semester_id = $2 AND sg.status = 'active'
      ...
      student_teacher_remarks AS (
        ...
        WHERE sr.class_id = $1 AND sr.semester_id = $2
      ),
      total_students AS (
      ...
      student_attendance AS (
        ...
        FROM attendance
        WHERE class_id = $1 AND semester_id = $2 AND status = 'present'
        GROUP BY student_id
      ),
      ...
      SELECT 
        ...
        json_object_agg(
          ssg.subject_name, 
          json_build_object('class_score', CASE WHEN ssg.class_score = 0 THEN '-' ELSE ssg.class_score::text END, 
      ...
      GROUP BY s.student_id, s.first_name, s.last_name, s.email, c.class_name, sa.average_score, sa.total_score, 
               ...
               sta.total_attendance, sea.total_semester_attendance
    ...
    return NextResponse.json(
      { students, totalStudents, totalSemesterAttendance, className },
      { status: 200 }
    );
  } catch (error) {
