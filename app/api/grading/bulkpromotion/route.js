...
export async function PUT(req) {
  ...
  try {
    ...
    // Update all students: set class_id to class_promoted_to
    const updateStudentsQuery = `
      UPDATE students
      SET class_id = class_promoted_to
      WHERE class_promoted_to IS NOT NULL
      RETURNING student_id, first_name, last_name, class_id, class_promoted_to;
    `;
    ...
    if (studentsResult.rowCount === 0 && semesterResult.rowCount === 0) {
      ...
      return NextResponse.json(
        { message: "Semester Closed successfully. If this is a promotional term, make sure to promote the students if neccessary before closing this term." },
        { status: 200 }
      );
    }
