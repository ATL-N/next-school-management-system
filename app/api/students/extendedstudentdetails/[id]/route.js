...
export async function GET(req, { params }) {
  ...
  try {
    ...
    `;
    const incidentResult = await db.query(incidentQuery, [student.user_id]);
    ...
    `;
    const attendanceResult = await db.query(attendanceQuery, [id, activesemester.semester_id]);
    ...
    `;
    const academicResult = await db.query(academicQuery, [id]);
    ...
    const remarksQuery = `
      ...
      LIMIT 1
    `;
    const remarksResult = await db.query(remarksQuery, [id]);
