...
const TeacherEvaluation = ({ evaluation_id, staff_id, staffData, pastEvaluation, onCancel }) => {
  ...
  useEffect(() => {
    ...
    const authorizedPermissions = [ "add evaluation", "update evaluation"];

    if (session?.user?.permissions?.some((permission) => authorizedPermissions.includes(permission))) {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prevState) => {
      ...
      if (["teaching_effectiveness", "classroom_management", "student_engagement", "professionalism"].includes(name)) {
        const ratings = [
  ...
  const handleConfirm = async () => {
    ...
    try {
      const url = evaluation_id ? `/api/staff/evaluation/${evaluation_id}` : "/api/staff/evaluation";
      const method = evaluation_id ? "PUT" : "POST";

      toast.update(toastId2, {
        render: evaluation_id ? "Updating evaluation..." : "Adding new evaluation...",
        type: "info",
        isLoading: true,
      });
      ...
      console.log(evaluation_id ? "Evaluation updated successfully:" : "Evaluation added successfully:", result);
      ...
      console.error(evaluation_id ? "Error updating evaluation:" : "Error adding evaluation:", error);
      toast.update(toastId2, {
  ...
  if (!isAuthorised) {
    return <div className="flex items-center">You are not authorised to be on this page</div>;
  }
